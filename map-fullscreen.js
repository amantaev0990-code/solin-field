...
window.toggleMapFullscreen=function(force){
  ...
  /* Leaflet must recalculate its canvas after the viewport changes. */
  setTimeout(()=>window.fieldMap?.invalidateSize?.({pan:false}),60);setTimeout(()=>window.fieldMap?.invalidateSize?.({pan:false}),260);
};
function install(){if(ensureButton())return;setTimeout(ensureButton,0);setTimeout(ensureButton,300)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install);else install();
window.addEventListener('orientationchange',()=>setTimeout(()=>window.fieldMap?.invalidateSize?.({pan:false}),250));
})();
