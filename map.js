let fieldMap=null,baseGoogle=null,baseOSM=null,userMarker=null,cmfExtent=null;
const cmf2Layers=[
...
function toggleCmfLayer(id,on){const x=cmf2Layers.find(v=>v.id===id);if(!x)return;if(x.leaflet){if(on)x.leaflet.addTo(fieldMap);else fieldMap.removeLayer(x.leaflet)}else if(on)toast(`${x.name}: слой найден в CMF2, геометрия готовится к переносу`)}
