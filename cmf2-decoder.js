/* CMF2 browser decoder v1: structural metadata only. No fabricated geometry. */
(function(){
  ...
  window.inspectCmf2=async function(file){
    ...
    const pstg=findAscii(u8,'PSTG');
    const sqlite=findAscii(u8,'SQLite format 3');
    const extent=readKnownExtent(view);
    return {layers,projection:proj||'UTM (проекция найдена не полностью)',pstgBlocks:pstg.length,sqliteHeaders:sqlite.length,extent,size:u8.length};
  };
  window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{const i=document.getElementById('mapFileInput');if(i)i.removeAttribute('accept')},0));
  const oldEnsure=window.ensureMapFileUi;
  if(typeof oldEnsure==='function')window.ensureMapFileUi=function(){oldEnsure();const i=document.getElementById('mapFileInput');if(i)i.removeAttribute('accept')};
})();
