...
(function(){
 ...
 function labelText(f){const p=f?.properties||{},q=read(p,['Nкварт','NКВАРТ','Квартал','KV']),v=read(p,['Nвыд','NВЫД','Выдел','VD']),a=read(p,['Площ','Площадь','PL','Площ_геом']);if(isStand(f)){return v==null?'':String(v)}if(isQuarter(f)){const id=q==null?'':String(q),area=a==null?'':`${String(a).replace('.',',')} га`;return[id,area].filter(Boolean).join('\n')}return''}
 ...
 window.bindImportedFeature=function(feature,layer){if((isStand(feature)||isQuarter(feature))&&layer.bindTooltip){const txt=labelText(feature);if(txt)layer.bindTooltip(txt,{permanent:true,direction:'center',className:isQuarter(feature)?'quarterPermanentLabel':'standPermanentLabel',interactive:false})}layer.on('click',e=>{resetSelection();selected={feature,layer};window.__cmfSelectedLayer=layer;if(layer.setStyle)layer.setStyle(selectedStyle(feature));if(layer.bringToFront)layer.bringToFront();if(typeof showCmfSheet==='function')showCmfSheet(feature,e?.latlng,layer);else layer.bindPopup('Объект').openPopup(e?.latlng)})};
